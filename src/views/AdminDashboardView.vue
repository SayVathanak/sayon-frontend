...
import { 
    ...
    Search, Store, MapPin, Loader2, ListPlus, UploadCloud, TrendingUp, Users, DollarSign, Menu, Coffee
...
onMounted(() => {
    // Mock auth check for demo purposes if store is empty
    if (auth.user?.role !== 'admin' && !auth.user) { 
        // In a real scenario, redirect. For UI demo, we allow render.
...
<template>
  <div class="flex flex-col md:flex-row h-screen bg-zinc-50 font-sans text-zinc-900 overflow-hidden antialiased selection:bg-red-100 selection:text-red-900">
    
    <!-- Mobile Top Header -->
    <header class="md:hidden flex items-center justify-between px-6 py-4 bg-white border-b border-zinc-200 z-30 shrink-0">
        <h1 class="text-lg font-black uppercase flex items-center gap-2 tracking-tight text-red-950">
            <div class="w-8 h-8 bg-red-800 text-white flex items-center justify-center rounded-lg shadow-md shadow-red-200">S</div>
            Sayon
        </h1>
        <button @click="handleLogout" class="text-zinc-400 hover:text-red-600 transition-colors">
            <LogOut size="20" />
        </button>
    </header>
    ...
    <aside class="hidden md:flex flex-col w-64 bg-white border-r border-zinc-200 h-full shrink-0 z-20">
        ...
        <div class="p-6 border-t border-zinc-100">
            <div class="flex items-center gap-3 mb-4">
                <div class="w-8 h-8 rounded-full bg-zinc-100 text-zinc-500 flex items-center justify-center font-bold text-xs">{{ auth.userName?.charAt(0) || 'A' }}</div>
                <div><p class="text-xs font-bold text-zinc-900">{{ auth.userName || 'Admin' }}</p><p class="text-[10px] text-zinc-400 font-bold uppercase">Administrator</p></div>
            </div>
    ...
    <!-- Main Content -->
    <main class="flex-1 overflow-y-auto p-4 md:p-8 relative pb-24 md:pb-8" ref="animationParent">
        
        <!-- Dashboard Tab -->
        <div v-if="activeTab === 'dashboard'" class="max-w-6xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <header>
                <h2 class="text-2xl md:text-3xl font-black tracking-tight text-zinc-900">Performance</h2>
                <p class="text-zinc-500 text-sm md:text-base font-medium">Real-time metrics from all branches.</p>
            </header>
            
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
                <div class="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-zinc-100 hover:shadow-lg hover:shadow-red-50 hover:border-red-100 transition-all group">
                    ...
                    <p class="text-zinc-500 text-xs font-bold uppercase tracking-widest">Revenue</p>
                    <p class="text-3xl md:text-4xl font-black text-zinc-900 mt-1">${{ stats.totalSales.toLocaleString() }}</p>
                </div>
                <div class="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-zinc-100 hover:shadow-lg hover:shadow-red-50 hover:border-red-100 transition-all group">
                    ...
                    <p class="text-zinc-500 text-xs font-bold uppercase tracking-widest">Orders</p>
                    <p class="text-3xl md:text-4xl font-black text-zinc-900 mt-1">{{ stats.totalOrders }}</p>
                </div>
                <div class="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-zinc-100 hover:shadow-lg hover:shadow-red-50 hover:border-red-100 transition-all group">
                    ...
                    <p class="text-zinc-500 text-xs font-bold uppercase tracking-widest">Top Item</p>
                    <p class="text-xl md:text-2xl font-black text-zinc-900 mt-1 truncate">{{ stats.topProduct }}</p>
                </div>
            </div>

            <div class="bg-white rounded-2xl shadow-sm border border-zinc-100 overflow-hidden">
                <div class="p-5 md:p-6 border-b border-zinc-50"><h3 class="font-bold text-lg text-zinc-900">Branch Leaderboard</h3></div>
        ...
        <div v-if="activeTab === 'products'" class="max-w-6xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <h2 class="text-2xl md:text-3xl font-black text-zinc-900 tracking-tight">Inventory</h2>
                <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    <div class="relative flex-1 md:w-64"><Search class="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400" size="16"/><input v-model="searchQuery" type="text" placeholder="Search..." class="w-full pl-9 pr-4 py-3 bg-white border border-zinc-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-red-100 focus:border-red-800 outline-none transition-all shadow-sm"></div>
                    <button @click="openAddModal" class="bg-red-800 text-white px-5 py-3 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-red-900 transition-colors shadow-lg shadow-red-100 flex items-center justify-center gap-2 active:scale-95 transform duration-100"><Plus size="16"/> Add New</button>
                </div>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div v-for="product in filteredProducts" :key="product.product_id" @click="startEdit(product)" class="bg-white p-4 rounded-2xl border border-zinc-100 shadow-sm hover:shadow-xl hover:shadow-red-50 hover:border-red-100 transition-all cursor-pointer group relative overflow-hidden active:scale-[0.98] duration-100">
                    ...
                    <button @click.stop="handleDeleteProduct(product.product_id, product.name)" class="absolute top-3 right-3 p-2 bg-white/90 rounded-lg text-zinc-400 hover:text-red-600 md:opacity-0 md:group-hover:opacity-100 opacity-100 transition-all hover:bg-red-50 shadow-sm z-10"><Trash2 size="16"/></button>
                </div>
            </div>
        </div>

        <!-- Branches Tab -->
        <div v-if="activeTab === 'settings'" class="max-w-4xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <h2 class="text-2xl md:text-3xl font-black text-zinc-900 tracking-tight">Branches</h2>
            <div class="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-zinc-100">
                ...
                <form @submit.prevent="handleCreateBranch" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    ...
                    <button type="submit" class="bg-zinc-900 text-white px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-red-800 transition-colors shadow-lg w-full md:w-auto">Create</button>
                </form>
            </div>
            <div class="space-y-3">
                <div v-for="branch in branchStore.branchList" :key="branch.user_id" class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-5 bg-white border border-zinc-100 rounded-2xl shadow-sm hover:shadow-md transition-all gap-4">
                    <div class="flex items-center gap-4"><div class="w-10 h-10 bg-red-50 text-red-800 rounded-xl flex items-center justify-center font-bold text-sm shrink-0">{{ branch.name.charAt(0) }}</div><div><p class="font-bold text-zinc-900">{{ branch.name }}</p><p class="text-xs font-mono text-zinc-400">ID: {{ branch.username }}</p></div></div>
                    <div class="flex items-center justify-between w-full sm:w-auto gap-4"><span class="px-3 py-1 bg-zinc-50 text-zinc-500 text-[10px] font-bold uppercase tracking-widest rounded-lg">{{ branch.role }}</span><button v-if="branch.role !== 'admin'" @click="branchStore.deleteBranch(branch.user_id)" class="w-8 h-8 flex items-center justify-center bg-zinc-50 text-zinc-400 hover:bg-red-50 hover:text-red-600 rounded-lg transition-colors"><Trash2 size="16" /></button></div>
                </div>
            </div>
        </div>
    </main>
    ...
    <nav class="md:hidden fixed bottom-0 w-full bg-white border-t border-zinc-200 flex justify-around p-2 z-40 pb-safe">
        <button @click="activeTab = 'dashboard'" :class="activeTab === 'dashboard' ? 'text-red-800 bg-red-50' : 'text-zinc-400'" class="flex flex-col items-center gap-1 p-3 rounded-xl transition-all w-20">
            <LayoutDashboard size="20" />
            <span class="text-[10px] font-bold uppercase tracking-wide">Home</span>
        </button>
        <button @click="activeTab = 'products'" :class="activeTab === 'products' ? 'text-red-800 bg-red-50' : 'text-zinc-400'" class="flex flex-col items-center gap-1 p-3 rounded-xl transition-all w-20">
            <ShoppingBag size="20" />
            <span class="text-[10px] font-bold uppercase tracking-wide">Items</span>
        </button>
        <button @click="activeTab = 'settings'" :class="activeTab === 'settings' ? 'text-red-800 bg-red-50' : 'text-zinc-400'" class="flex flex-col items-center gap-1 p-3 rounded-xl transition-all w-20">
            <Store size="20" />
            <span class="text-[10px] font-bold uppercase tracking-wide">Store</span>
        </button>
    </nav>

    <!-- Modal: Bottom Sheet on Mobile, Centered on Desktop -->
    <div v-if="showProductModal" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
        <div class="absolute inset-0 bg-zinc-900/60 backdrop-blur-sm transition-opacity" @click="showProductModal = false"></div>
        <div class="relative w-full max-w-lg bg-white rounded-t-3xl sm:rounded-3xl p-6 sm:p-8 shadow-2xl animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-6 sticky top-0 bg-white z-10 pb-2 border-b border-transparent">
                <h3 class="font-black text-2xl text-zinc-900 tracking-tight">{{ isEditing ? 'Edit Item' : 'New Item' }}</h3>
                <button @click="showProductModal = false" class="p-2 bg-zinc-50 rounded-full hover:bg-red-50 hover:text-red-600 transition-colors"><X size="20"/></button>
            </div>
            
            <form @submit.prevent="handleSubmit" class="space-y-5">
                <div class="flex flex-col sm:flex-row gap-4">
                    <div class="w-full sm:w-24 h-32 sm:h-24 bg-zinc-50 rounded-2xl border border-dashed border-zinc-300 flex items-center justify-center overflow-hidden relative cursor-pointer hover:border-red-800 transition-colors shrink-0">
                ...
                <div class="bg-zinc-50 p-4 rounded-xl border border-zinc-100">
                    ...
                    <div class="flex flex-col sm:flex-row gap-2 mb-3">
                        <input v-model="newOptionName" placeholder="Name (e.g. Sugar)" class="flex-1 bg-white border border-zinc-200 rounded-lg px-3 py-3 sm:py-2 text-xs font-medium focus:border-red-800 outline-none"/>
                        <input v-model="newOptionValues" placeholder="Values (0%, 50%)" class="flex-1 sm:flex-2 bg-white border border-zinc-200 rounded-lg px-3 py-3 sm:py-2 text-xs font-medium focus:border-red-800 outline-none"/>
                        <button type="button" @click="addOptionGroup" class="bg-zinc-900 text-white p-3 sm:p-2 rounded-lg hover:bg-red-800 transition-colors flex items-center justify-center"><Plus size="16"/></button>
                    </div>
                ...
                <div class="flex flex-col-reverse sm:flex-row justify-between items-center gap-4 pt-2 pb-6 sm:pb-0">
                    <label class="flex items-center gap-2 cursor-pointer w-full sm:w-auto justify-center sm:justify-start"><input type="checkbox" v-model="productForm.is_available" class="w-5 h-5 accent-red-800 rounded"/><span class="text-xs font-bold uppercase tracking-wide text-zinc-500">Available for Sale</span></label>
                    <button type="submit" class="w-full sm:w-auto bg-red-800 text-white px-8 py-4 sm:py-3 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-red-900 transition-colors shadow-lg shadow-red-200">Save Item</button>
                </div>
            </form>
        </div>
    </div>
...
<style>
/* Utility for iPhone home indicator spacing */
.pb-safe {
