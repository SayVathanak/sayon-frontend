...
// Optimization: Only animate specific lists, not the whole page
...
// 3. HELPER: Image Optimization
function getThumbnail(url) {
    if (!url) return '';
    // If using Cloudinary, resize to 200px for speed
...
async function handleFileUpload(event) {
    ...
    formData.append('upload_preset', 'ml_default'); // Check your Cloudinary preset name
    try {
...
<template>
    <div class="flex flex-col md:flex-row h-screen bg-[#F8F9FD] font-sans text-slate-800 overflow-hidden antialiased selection:bg-blue-100 selection:text-blue-900">
        ...
        <aside class="hidden md:flex flex-col w-72 bg-white border-r border-slate-100 h-full shrink-0 z-20 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
            <div class="p-8 pb-4">
        ...
        <main class="flex-1 overflow-y-auto p-6 md:p-10 relative pb-24 md:pb-10">
                ...
                <div class="bg-white p-8 rounded-4xl shadow-sm border border-slate-100 mt-8">
                    ...
                    <div class="overflow-hidden rounded-2xl border border-slate-100">
                        <table class="w-full text-left">
                            ...
                            <tbody class="text-sm divide-y divide-slate-50" ref="salesTableRef">
        ...
        <div v-if="showProductModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-lg bg-white rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95 duration-200">
                <h3 class="font-black text-2xl text-slate-900 mb-6">{{ isEditing ? 'Edit Product' : 'New Product' }}</h3>
                <form @submit.prevent="handleSubmit" class="space-y-6">
                    <div class="flex gap-6">
                        <div class="w-24 h-24 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 flex items-center justify-center relative overflow-hidden shrink-0 hover:border-blue-500 transition-colors cursor-pointer group">
                    ...
                    <div class="bg-slate-50 p-5 rounded-3xl">
                        ...
                        <div class="flex gap-2 mb-3">
                            ...
                            <button type="button" @click="addOptionGroup" class="bg-blue-600 text-white p-2 rounded-xl"><Plus size="16" /></button>
                        </div>
                    ...
                    <div class="flex justify-between items-center pt-2">
                        <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" v-model="productForm.is_available" class="w-5 h-5 rounded text-blue-600 focus:ring-0" /><span class="text-sm font-bold text-slate-500">Available</span></label>
                        <div class="flex gap-3">
                            <button type="button" @click="showProductModal = false" class="px-6 py-3 rounded-2xl font-bold text-slate-400 hover:bg-slate-50 transition-colors">Cancel</button>
                            <button type="submit" class="bg-blue-600 text-white px-8 py-3 rounded-2xl font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 transition-colors">Save</button>
                        </div>
                    </div>
                </form>
            </div>
